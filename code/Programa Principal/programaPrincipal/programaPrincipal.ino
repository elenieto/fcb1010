...
class MyCallbacks: public BLECharacteristicCallbacks {
    ...
    void onWrite(BLECharacteristic *pCharacteristic) {
      ...
      if (value.length() > 0) {
        const size_t capacity = JSON_OBJECT_SIZE(22) + 140;
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (value > lastValueExp[i] + 5 || value < lastValueExp[i] - 5) {
      ...
      if (state == 4) {
        ...
        if (i == 1) {
          indexCC = expChange ? 19 : 21;
        } else {
          indexCC = 20;
        }
        midiPacket[2] = freeExp[i][0];
...
void printlcd(int progMode) {
  ...
  if (progMode != 4) {
    lcd.setCursor(0, 1);
    if (expChange) lcd.print("WAH     VOLUM   ");
    else lcd.print("WAH     PITCH   ");
  }
  printLcd = false;
}
